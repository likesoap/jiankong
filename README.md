...
> 均可按相同思路部署。
>
> **说明**: 本文以 N100 类小主机 (Intel N100/N95/J4125 等) 作为
> 网关宿主机代表，下文统一简称为 **N100**。任何 x86_64/ARM64 Linux 设备
> (树莓派、R4S、旧笔记本) 均可替代，对硬件无特殊绑定。
>
### 硬件
...
```
┌──────────┐     ┌──────────────────────────┐     ┌──────────┐     ┌──────┐
│ ISP 光猫  │ ──→ │ N100 eth0 (WAN)          │ ──→ │ N100     │ ──→ │ Cudy │
│          │     │ DHCP 客户端               │     │ eth1 LAN │     │ AP   │
└──────────┘     │                          │     │ .1/24    │     │ .2   │
                 │ ┌──────────────────────┐ │     └────┬─────┘     └──┬───┘
                 │ │ Docker               │ │          │              │
                 │ │ AdGuard Home :53 :80 │ │     ┌────┴──────────────┴───┐
                 │ │ ntopng       :3001   │ │     │ WiFi + 有线设备       │
                 │ └──────────────────────┘ │     └────────────────────────┘
                 │                          │
                 │ nftables                 │
                 │  NAT + DNS劫持           │
